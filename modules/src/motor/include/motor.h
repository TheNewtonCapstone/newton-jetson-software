...
class MotorDriver : public rclcpp::Node {
 public:
  ...
  result<void> set_joint_position(const joint::id joint_id, const float position);
  result<void> set_joint_velocity(const joint::id joint_id, const float velocity);
  result<void> set_joint_torque(const joint::id joint_id, const float torque);
  result<void> set_joints_positions(const std::array<float, NUM_JOINTS> positions);
 ...
 private:
  ...
  static constexpr uint8_t NUM_JOINTS = NUM_LEGS * NUM_JOINTS_PER_LEG; // 12 joints used interchangeably with motors with motors
  ...
  std::array<rclcpp::Publisher<odrive_can::msg::ControllerStatus>::SharedPtr, NUM_JOINTS> joint_pubs;
  std::array<rclcpp::Subscription<odrive_can::msg::ControlMessage>::SharedPtr, NUM_JOINTS> joint_subs;
  std::array<rclcpp::Client<odrive_can::srv::AxisState>::SharedPtr, NUM_JOINTS> joint_srvs;
