...
async def main():
    ...
    parser.add_argument('-b', '--bitrate', type=int, default=1000000, help='Bitrate for CAN bus. Default is 1 Mb.')
    parser.add_argument('--node-id', type=int, help='CAN Node ID of the ODrive.')
    parser.add_argument('--endpoints-json', type=str, required=True,
    ...
    with can.interface.Bus(args.channel, bustype=args.interface, bitrate=args.bitrate) as bus:
        ...
        while driver_id < max_id:
            with CanSimpleNode(bus=bus, node_id=driver_id) as node:
                ...
                if args.save_config:
                    print(f"saving configuration to node {driver_id}...")
